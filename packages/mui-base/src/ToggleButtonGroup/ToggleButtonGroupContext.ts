 ...
 */
const Context = React.createContext<ToggleButtonGroupContextValue | null>(null);

if (process.env.NODE_ENV !== 'production') {
  Context.displayName = 'ToggleButtonGroupContext';
}

export function useToggleButtonGroupContext() {
  const context = React.useContext(Context);
  if (context == null) {
    throw new Error('No ToggleButtonGroupContext provided');
  }
